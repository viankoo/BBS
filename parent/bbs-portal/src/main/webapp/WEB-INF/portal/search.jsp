			...
			<div class="sl-b-selected J_brandSelected">
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${map }" var="m">
						<a title="依琦莲（yiqilian）" id="${m.key }"  href="javascript:showBack('${m.key }');" class="crumb-select-item">
...
<script>
	function fqBrand(brandId){
		location = '/search?keyword=${keyword}&brandId='+brandId+'&pa=${pa}&pb=${pb}&sort=${sort0}&pageNum=${pageNum}&pageSize=6';
	}
	...
	function showBack(key){
		if(key=="品牌"){
			location = '/search?keyword=${keyword}&brandId=&pa=${pa}&pb=${pb}&sort=${sort0}&pageNum=${pageNum}&pageSize=6';
		}
		if(key=="价格"){
			location = '/search?keyword=${keyword}&brandId${brandId}=&pa=&pb=&sort=${sort0}&pageNum=${pageNum}&pageSize=6';
		}
	}
</script>
		...
		<div class="ml-wrap">
			<div id="J_filter" class="filter">
				<div class="f-line top">
					<div class="f-sort"><a href="javascript:;" class="curr">综合排序<i></i>
					</a><a href="javascript:;" class="">销量<i></i>
					</a><a href="javascript:serachKeyword('${sort }',1);" class="">价格<i></i>
					</a><a href="javascript:;" class="" >评论数<i></i>
					</a><a href="javascript:;" class="">新品<i></i></a></div>
					<div class="f-search">
					...
					<div id="J_topPage" class="f-pager"><span class="fp-text"><b>${superProducts.pageNum }</b><em>/</em>
					<i>${superProducts.pages }</i></span><a class="fp-prev disabled" href="javascript:serachKeyword('${sort0}','${superProducts.pageNum-1<1?1:superProducts.pageNum-1}');">&lt;</a>
					<a class="fp-next" href="javascript:serachKeyword('${sort0}','${superProducts.pageNum+1>superProducts.pages?superProducts.pages:superProducts.pageNum+1}');" title="使用方向键右键也可翻到下一页哦！">&gt;</a></div>
					<div class="f-result-sum">共<span id="J_resCount" class="num">${superProducts.total }</span>件商品</div>
					<span class="clr"></span>
				</div>
				<div class="f-line">
					<div class="f-store">
...
<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
	<ul class="gl-warp clearfix" data-tpl="1">
		<c:forEach items="${superProducts.result }" var="product">
